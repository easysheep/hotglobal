...
const CategoryPage = () => {
  ...
  const handleSearch = (e) => {
    if (e.key === 'Enter') {
      ...
      if (searchQuery.startsWith('#')) {
        ...
        // Existing code
        const page = pages.find(p => p.title.toLowerCase() === searchQuery.toLowerCase());
        if (page) {
          router.push(`/blogpage?title=${encodeURIComponent(page.title)}&followers=${page.followers}`);
        } else {
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 text-5xl font-semibold pt-6">
        <div className="">Posts For </div>
        <div className="font-black text-purple1">{category}</div>
      </div>
